...
| 改整理记录 / 重新整理 | `internal/api/orgrecord.go`；路径推导在纯函数 `planRedoLayout`、原地刷新判定在 `isInPlaceRedo`，配套测试 `orgrecord_test.go`。**没改过名的文件（记录里无 `Orig`，即未识别 / 失败的）必须走 `redoParseVideo` → `parseVideoInDir`**，和正常整理同一套（替换规则 → 子目录季号 → 条目目录季号，子目录取自 `orgRecordFile.Dir`）；只用 `parseFileName` 的话各季同名的 E01.mkv 会撞名。**改 `redoOrganize` 前先读它的步骤注释**：算布局 → 动网盘 → 删旧本地产物 → 落盘，这个顺序是有来由的，破坏性动作必须排在计算之后 |
