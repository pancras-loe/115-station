...
| **网盘文件页** | `filebrowser.go` `filescrape.go` `fileorganize.go` | 浏览 115 目录树（`GET /files/115`），对勾选条目「刮削」（任务 `scrape`）或「整理」（任务 `orgpick`），都入任务队列。刮削核心是 `scrape.go` 的 `scrapeTitleMeta` + `metaWriter`：媒体库里的按台账写本地、可选这一次上传网盘；库外的只能直接写进网盘目录。「上传」只管这一次，走 `upload115FileConsented`（不看监控上传总开关），不上传时把本地产物登记成已上传，免得监控上传随后自己传上去。整理复用 `processEntry`，媒体库内的条目拒收（走重新整理）。测试 `filebrowser_test.go` |
...
| 改网盘文件页（手动刮削 / 整理所选） | 后端 `internal/api/filebrowser.go`（列目录、定位、入队参数）/ `filescrape.go` / `fileorganize.go`；前端 `webui/src/pages/FilesPage.vue` + `webui/src/components/files/`。新增会写网盘的分支记得走 `ops.*`（抑制、缓存失效），上传只在用户这一次勾了才能用 `upload115FileConsented` |
