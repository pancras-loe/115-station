...
## 6. 阶段 3 ｜拉取层重写（P0 + P1）— ✅ 已完成（2026-09-19）

> 落地情况：新增 [`life115.go`](internal/api/life115.go)（端点选择 + 405 粘滞降级 +
> 游标分页 + 忽略类过滤 + file_id 去重 + 开关门禁）与
> [`life115_test.go`](internal/api/life115_test.go)（13 个用例）；
> `files115.go` 加 `httpStatusError` / `isHTTPStatus`；
> `ratelimit.go` 加 `throttleLife` 独立 2s 队列；
> `markEventsCoveredByFullSync` 从「灌 1000 条 applied 行」改成
> 「标掉 pending + 推游标」，请求数 34 → 1。
>
> **与原计划的差异**：`lifeIgnoreTypes` 与 file_id 去重原本排在阶段 4，
> 实施时并入本阶段 —— 它们就长在翻页循环里，拆开等于把同一个循环写两遍。
> 阶段 4 因此只剩 `SyncEvent` 加列。
>
> 变异验证两条：摘掉 `reachedCursor` 的整轮返回 → 游标用例失败（实得 5 条而非 2 条）；
> 摘掉 405 判定 → 降级用例失败。
...
## 9. 阶段 6 ｜兜底收紧（P2）— ✅ 已完成（2026-09-19）

> 落地情况：`removeSyncedItem` 加 `libName` 参数，第 2 级路径推导补上库名前缀；
> `dirRel`/`dirOK` 提到函数开头算一次，第 2 级与第 4 级共用；
> 第 4 级的 `WalkDir` 从 `localRoot` 收窄到 `dirRel` 子树，推不出范围就放弃。
> 新增 [`incr_remove_test.go`](internal/api/incr_remove_test.go)（5 个用例）。
>
> 两条变异验证：第 2 级去掉库名前缀 → 定位用例失败；第 4 级恢复全库扫描 →
> 「范围外同名文件被误删」与「推不出范围时不该报成功」两条同时失败。
> 后者的失败输出直接证明了旧行为确实会删掉另一棵子树下的同名文件。
...
## 10. 阶段 7 ｜独立轮询（P2）— ✅ 已完成（2026-09-19）

> 落地情况：`incr.interval_sec` 默认 30 秒（用指针区分「没配过」与「显式填 0」，
> 老用户升级上来不会被当成关闭）；`startIncrPoller` 每轮重读间隔，改配置不必重启；
...
> 不再等下一个 cron 周期。cron 被清空时一并清掉标记。
>
> 轮询轮次**不 beginTask**：30 秒一次会让前端「当前任务」闪个不停、把运行历史刷满。
> 后台轮询的可见性交给阶段 8 的状态页，不占「当前任务」这个位置。
...
## 11. 阶段 8 ｜状态页（P2）— ✅ 已完成（2026-09-19）

> 落地情况：新增 [`incrstatus.go`](internal/api/incrstatus.go)
> （`GET /sync/incr-status` + `POST /sync/incr-probe`）与
> [`incrstatus_test.go`](internal/api/incrstatus_test.go)；
> `executeIncrementalSyncWith` 改用具名返回值，每轮（含熔断与失败）
> 都往 `noteIncrRound` 记一份快照。
> 前端 [`IncrSyncTab.vue`](webui/src/pages/strm/IncrSyncTab.vue) 加「事件流状态」卡片
> 与「测试事件流」按钮。
>
> 探针是纯读：不推进游标、不落库、不动本地文件，但会顺手打开「115 生活」开关 ——
> 开关关着时一定拉到 0 条，而用户看到「0 条」是分不清「没有变动」和「开关没开」的。
>
> 顺带改掉了「这条 cron 管什么」那张卡片：它还在说「整理与增量绑在同一条 cron 上」，
> 阶段 7 之后已经不成立。
